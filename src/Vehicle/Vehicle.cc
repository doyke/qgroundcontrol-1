...
void Vehicle::_sendMessageOnLink(LinkInterface* link, mavlink_message_t message)
{
    ...
    mavlink_finalize_message_chan(&message, _mavlink->getSystemId(), _mavlink->getComponentId(), link->getMavlinkChannel(), message.len, message.len, messageKeys[message.msgid][1]);
...
QString Vehicle::vehicleTypeName() const {
    ...
    typeNames.insert( MAV_TYPE_COAXIAL,         tr("Coaxial helicopter"));
    typeNames.insert( MAV_TYPE_HELICOPTER,      tr("Normal helicopter with tail rotor."));
    typeNames.insert( MAV_TYPE_ANTENNA_TRACKER, tr("Ground installation"));
    typeNames.insert( MAV_TYPE_GCS,             tr("Operator control unit / ground control station"));
    typeNames.insert( MAV_TYPE_AIRSHIP,         tr("Airship, controlled"));
    typeNames.insert( MAV_TYPE_FREE_BALLOON,    tr("Free balloon, uncontrolled"));
    typeNames.insert( MAV_TYPE_ROCKET,          tr("Rocket"));
    typeNames.insert( MAV_TYPE_GROUND_ROVER,    tr("Ground rover"));
    typeNames.insert( MAV_TYPE_SURFACE_BOAT,    tr("Surface vessel, boat, ship"));
    typeNames.insert( MAV_TYPE_SUBMARINE,       tr("Submarine"));
    typeNames.insert( MAV_TYPE_HEXAROTOR,       tr("Hexarotor"));
    typeNames.insert( MAV_TYPE_OCTOROTOR,       tr("Octorotor"));
    typeNames.insert( MAV_TYPE_TRICOPTER,       tr("Octorotor"));
    typeNames.insert( MAV_TYPE_FLAPPING_WING,   tr("Flapping wing"));
    typeNames.insert( MAV_TYPE_KITE,            tr("Flapping wing"));
    typeNames.insert( MAV_TYPE_ONBOARD_CONTROLLER, tr("Onboard companion controller"));
    typeNames.insert( MAV_TYPE_VTOL_DUOROTOR,   tr("Two-rotor VTOL using control surfaces in vertical operation in addition. Tailsitter"));
    typeNames.insert( MAV_TYPE_VTOL_QUADROTOR,  tr("Quad-rotor VTOL using a V-shaped quad config in vertical operation. Tailsitter"));
    typeNames.insert( MAV_TYPE_VTOL_TILTROTOR,  tr("Tiltrotor VTOL"));
    typeNames.insert( MAV_TYPE_VTOL_RESERVED2,  tr("VTOL reserved 2"));
    typeNames.insert( MAV_TYPE_VTOL_RESERVED3,  tr("VTOL reserved 3"));
    typeNames.insert( MAV_TYPE_VTOL_RESERVED4,  tr("VTOL reserved 4"));
    typeNames.insert( MAV_TYPE_VTOL_RESERVED5,  tr("VTOL reserved 5"));
    typeNames.insert( MAV_TYPE_GIMBAL,          tr("Onboard gimbal"));
    typeNames.insert( MAV_TYPE_ADSB,            tr("Onboard ADSB peripheral"));
#endif
